...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState("events"); // For section tabs
  ...
  const fetchMembers = async (clubId) => {
    let { data } = await supabase
      .from("memberships")
      .select("id, joined_at, user_id, auth_user: user_id ( email )")
      .eq("club_id", clubId);
    setMembers(data || []);
  };
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="section">
        <div className="collapsible-header" onClick={() => toggleSection('club')}>
          <h3>Club Profile</h3>
          <span className={`toggle-icon ${expandedSections.club ? 'expanded' : ''}`}>▼</span>
        </div>
        <div className={`collapsible-content ${expandedSections.club ? 'expanded' : ''}`}>
      ...
      {club && (
        <>
          {/* Events Section */}
          <div className="section">
            <div className="collapsible-header" onClick={() => toggleSection('events')}>
              <h3>Manage Events ({events.length})</h3>
              <span className={`toggle-icon ${expandedSections.events ? 'expanded' : ''}`}>▼</span>
            </div>
            <div className={`collapsible-content ${expandedSections.events ? 'expanded' : ''}`}>
          ...
          <div className="section">
            <div className="collapsible-header" onClick={() => toggleSection('announcements')}>
              <h3>Manage Announcements ({announcements.length})</h3>
              <span className={`toggle-icon ${expandedSections.announcements ? 'expanded' : ''}`}>▼</span>
            </div>
            <div className={`collapsible-content ${expandedSections.announcements ? 'expanded' : ''}`}>
              <form onSubmit={addAnnouncement}>
                ...
                <button type="submit" className="form-submit">Add Announcement</button>
              </form>
          ...
          {/* Members Section */}
          <div className="section">
            <div className="collapsible-header" onClick={() => toggleSection('members')}>
              <h3>Club Members ({members.length})</h3>
              <span className={`toggle-icon ${expandedSections.members ? 'expanded' : ''}`}>▼</span>
            </div>
            <div className={`collapsible-content ${expandedSections.members ? 'expanded' : ''}`}>
              <div className="items-list">
                {members.map((m) => (
                  <div key={m.id} className="item-card member-card">
                    <h4 className="item-title">{m.auth_user?.email}</h4>
                    <p className="item-details">
                      Joined: {new Date(m.joined_at).toLocaleDateString()}
                    </p>
                    <button 
                      className="delete-btn" 
                      onClick={() => removeMember(m.id, m.auth_user?.email)}
                    >
                      Remove
                    </button>
                  </div>
